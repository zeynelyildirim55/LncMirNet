...
for pair in all_pairs:
    ...
    H, xedges, yedges = np.histogram2d(lnc_ver, mir_vec, bins=bin_size, density=True)
    H_kmer = H[:, :, np.newaxis]
...
accs, f1s, sens, spes, mccs, aucs = [], [], [], [], [], []

kf = KFold(n_splits=5, shuffle=True, random_state=42)

for fold_no, (train_index, test_index) in enumerate(kf.split(list(range(len(all_mats_np))))):
    fold_id = fold_no + 1
    print(f"\n========== STARTING FOLD {fold_id} ==========")

    #checkpoint logic cause this code does not have any checkpoint logic???

    checkpoint_path = f"./data/best_model_fold_{fold_id}.h5"
    ...
    model = get_model()
    adma = optimizers.Adam(learning_rate=0.000001)
    model.compile(optimizer=adma, loss=losses.binary_crossentropy, metrics=['accuracy'])

    
    if os.path.exists(checkpoint_path):
        print(f"--> Found existing model for Fold {fold_id}! Skipping training and evaluating directly.")
        model.load_weights(checkpoint_path)
    else:
        print(f"--> No previous model found for Fold {fold_id}. Training from scratch...")
        ...
        # Load the absolute best weights achieved during this training session
        if os.path.exists(checkpoint_path):
            model.load_weights(checkpoint_path)

    # Proceed to predict and evaluate (This now happens whether it trained or skipped!)
    ...
    confusion = metrics.confusion_matrix(test_label_true, res_label)
    ...
    acc = metrics.accuracy_score(y_true=test_label_true, y_pred=res_label)
    f1_score = metrics.f1_score(y_true=test_label_true, y_pred=res_label)
    sensitivity = TP / float(FN + TP)
    specificity = TN / float(TN + FP)
    mcc = metrics.matthews_corrcoef(y_true=test_label_true, y_pred=res_label)
    auc = metrics.roc_auc_score(y_true=test_label_true, y_score=res_pred_s)

    print(f"Fold {fold_id} Results:")
    print(f"  ACC: {acc:.4f} | AUC: {auc:.4f} | F1: {f1_score:.4f}")
